...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet) = {
    val leftFiltered = that.filter(_.text < elem.text)
    val rightFiltered = that.filter(_.text > elem.text)
    val leftSubtree = left.union(leftFiltered)
    val rightSubtree = right.union(rightFiltered)
    new NonEmpty(elem, leftSubtree, rightSubtree)
   }

  def mostRetweeted = mostRetweetedTraverse(elem)

  def mostRetweetedTraverse(tweet: Tweet) = left.mostRetweetedAcc(right.mostRetweetedAcc(tweet))

  def mostRetweetedAcc(acc: Tweet) =
    if (elem.retweets > acc.retweets)
      mostRetweeted
    else
      mostRetweetedTraverse(acc)

  def descendingByRetweet = {
    val top = mostRetweeted
    val remainder = remove(top)
    new Cons(top, remainder.descendingByRetweet)
  }
...
object Main extends App {
  ...
  println(GoogleVsApple.googleTweets union GoogleVsApple.appleTweets)
